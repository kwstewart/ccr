			...
			echo '<div class="container">';
				echo '<div id="banner" class="sub-header-inner '.$class_con.'">';
					...
					echo '<header class="banner-header '. $class_left .'">';
						...
						echo '<'. composer_title_tag( $title_tag ) .' class="sub-banner-title">' . esc_html( $title ) . '</'. composer_title_tag( $title_tag ) .'>';
					echo '</header>';
			...
			if ( is_category() ) {
				...
				if ( get_post_type() != 'post' ) {
					...
					if ( is_singular( 'pix_portfolio' ) ){
						...
						echo '<li><a href="' . esc_url( $portfolio_page_url ) . '">' . esc_html__( 'Portfolio', 'composer' ) . '</a> ' . $delimiter . '</li>';
					}
...
if( ! function_exists( 'composer_get_page_title' ) ) {	
	function composer_get_page_title() {
		...
		if ( !is_front_page() && !is_home() && !is_single() && !is_page() && get_post_type() != 'post' && !composer_bbp_is_user_home() && !is_search() && !is_post_type_archive( 'tribe_events' ) && !composer_is_shop() && !is_404() ) {
