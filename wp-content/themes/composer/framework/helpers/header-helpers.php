...
function composer_social_icons() {
	...
	if( !empty( $facebook ) || !empty( $twitter ) || !empty( $gplus ) || !empty( $linkedIn ) || !empty( $dribble ) || !empty( $flickr ) || !empty( $pinterest ) || !empty( $tumblr ) || !empty( $youtube ) || !empty( $vimeo ) || !empty( $rss ) || !empty( $instagram ) ) {
		...
		if( !empty($vimeo )) {
			$social_icons .= '<a href="'. esc_url( $vimeo ) .'" target="_blank" title="Vimeo" class="vimeo"><i class="pixicon-vimeo"></i></a>';
		}
			...
			$output .= '<a href="'. esc_url( home_url( '/' ) ) .'"'. $mobile_logo_class .' rel="nofollow">';
				...
				if ( $custom_logo != get_bloginfo( 'name' ) || $custom_logo_light != get_bloginfo( 'name' ) ) {
					...
					if ( $custom_sticky_logo != '' ) { 
						...
						$output .= '<img src="'. esc_url( $custom_sticky_logo ) .'"'. $custom_sticky_logo2x .' alt="" class="sticky-logo">';
					}
					if ( $custom_mobile_logo != '' ) { 
