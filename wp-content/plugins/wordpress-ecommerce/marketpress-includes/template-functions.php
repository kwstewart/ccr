...
function _mp_cart_login($echo = false) {
  ...
  if ( is_user_logged_in() || defined('MP_HIDE_LOGIN_OPTION') ) {
    ...
    $content .= '</form>';
    $content .= 'Not registered? <a href="'.get_bloginfo('siteurl').'/wp-login.php?action=register/">Register here</a>.';
    $content .= '</td>';
...
function _mp_cart_shipping($editable = false, $echo = false) {
  ...
  if ($editable) {
    ...
    $content .= '<thead><tr>';
    $content .= '<th colspan="2" style="padding-left:30px">'.($mp->download_only_cart($mp->get_cart_contents()) ? __('Enter Your Checkout Information:', 'mp') : __('Enter Your Shipping Information:', 'mp')).'</th>';
    $content .= '</tr></thead>';
    $content .= '<tbody>';
    $content .= '<tr>';
    ...
    $content .= '<table class="mp_cart_shipping">';
    $content .= '<thead><tr>';
    $content .= '<th style="padding-left:30px">'.__('Shipping Information:', 'mp').'</th>';
...
function mp_list_products_grid( $echo = true, $paginate = '', $page = '', $per_page = '', $order_by = '', $order = '', $category = '', $tag = '' ) {
  global $wp_query, $mp;
  $settings = get_option('mp_settings');
  ...
  if ($last = count($custom_query->posts)) {
    ...
    foreach ($custom_query->posts as $post) {
			...
			//add last css class for styling grids
			if ($count == $last)
			  $class = array('mp_product grid_3', 'last-product');
			else
			  $class = 'mp_product grid_3';

		
      $content .= '<div '.mp_product_class(false, $class, $post->ID).'>';
      $content .= '<div class="mp_product_content_grid">';
   
      $product_content = mp_product_image( false, 'list', $post->ID );
      //$product_content .= $mp->product_excerpt($post->post_excerpt, $post->post_content, $post->ID);
      
      // Truncate the title down to 25 chars
      $shortTitle = strlen($post->post_title) > 20 ? substr($post->post_title,0,20)."..." : $post->post_title;
  
      $content .= apply_filters( 'mp_product_list_content', $product_content, $post->ID );
      $content .= '<div class="product_name"><h3 class="mp_product_name"><a href="' . get_permalink( $post->ID ) . '">' . $shortTitle . '</a></h3></div>';
...
function mp_product_price( $echo = true, $post_id = NULL, $label = true ) {
  ...
  if ((is_array($meta["mp_price"]) && count($meta["mp_price"]) == 1) || !empty($meta["mp_file"])) {
    if ($meta["mp_is_sale"] && $meta["mp_sale_price"][0]) {
	    $price = '<span class="mp_current_price">'.$mp->format_currency('', $meta["mp_sale_price"][0]).'</span></span><br />';
	    $price .= '<span class="mp_special_price">Regular: <del class="mp_old_price">'.$mp->format_currency('', $meta["mp_price"][0]).'</del>';
	  } else {
  ...
  //show only if price is more than $0
  if($meta["mp_price"][0]!="0.00") $price = apply_filters( 'mp_product_price_tag', '<span class="mp_product_price">' . $price . '</span>', $post_id, $label );
		else $price="";
 ...
 * @param sting $label A label to prepend to the price. Defaults to "Price: "
 */
function mp_product_price_single( $echo = true, $post_id = NULL, $label = true ) {
  global $id, $mp;
  $post_id = ( NULL === $post_id ) ? $id : $post_id;
  ...
  //show only if price is more than $0
  if($meta["mp_price"][0]!="0.00") $price = apply_filters( 'mp_product_price_tag', '<span class="mp_product_price_big">' . $price . '</span>', $post_id, $label );
		else $price="";
...
function mp_buy_button( $echo = true, $context = 'list', $post_id = NULL ) {
  ...
  foreach ($meta as $key => $val) {
	...
	//check that price is not $0.00

	if($meta["mp_price"][0]!="0.00") {
  //check stock
  $no_inventory = array();
  $all_out = false;
...
function mp_buy_button_single ( $echo = true, $context = 'list', $post_id = NULL ) {
  global $id, $mp;
  $post_id = ( NULL === $post_id ) ? $id : $post_id;
  ...
  if ($product_link = $meta['mp_product_link']) {
    ...
    if ($all_out) {
      $button .= '<span class="mp_no_stock">' . __('Out of Stock', 'mp') . '</span>';
	        ...
	        $button .= '<span class="mp_quantity"><label><strong>' . __('Quantity:', 'mp') . '</strong> <input class="mp_quantity_field" type="text" size="1" name="quantity" value="1" /></label></span>&nbsp;';
	        $button .= '<br /><span class="mp_quantity"><label><strong>Availability:</strong> In Stock</label></span>&nbsp;';

			    $button .= '<input type="hidden" name="product_id" value="' . $post_id . '" />';
	        $button .= '<input type="hidden" name="action" value="mp-update-cart" />';
	        $button .= '<input class="button_addcart" type="submit" name="addcart" value="" />';
...
function mp_product_image( $echo = true, $context = 'list', $post_id = NULL, $size = NULL ) {
  ...
  if ($context == 'list') {
  ...
  }  else if ($context == 'single_product') {
    //size
    $size = array(200, 200);
    ...
    $title = __('View Larger Image of '.esc_attr($post->post_title), 'mp');
    $class = ' class="mp_product_image_link mp_lightbox"';
